 ...
 *   * Give the header column cssClass as well
 */
(function ($) {
  // register namespace
  $.extend(true, window, {
  ...
  function CheckboxSelectColumn(options) {
    ...
    function handleKeyDown(e, args) {
      if (e.which == 32) {
        if (_grid.getColumns()[args.cell] != null && _grid.getColumns()[args.cell].id === _options.columnId) {
    ...
    function handleHeaderCellRendered(e, args) {
      if (args.column.id == _options.columnId) {
        if (_options.cssClass != null) {
          $(args.node).addClass(_options.cssClass);
        }
